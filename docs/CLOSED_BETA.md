...
технические JID не попадают в очередь. `sales_agent_next_action(lane="inbox")` возвращает
только незавершённое интервью или работу по входящим. Второй чат вызывает
`sales_agent_next_action(lane="prospecting")` и получает только компании для исследования,
анализа, scoring и черновиков. Оба чата используют общую CRM, но не могут забрать работу друг друга.
...
`sales_submit_conversation_decision`. Для задач внутри двух чатов используйте ежечасное
расписание или ручной запуск и точные prompts из `sales_get_chatgpt_agent_playbook`;
15-минутная часть на сервере только
